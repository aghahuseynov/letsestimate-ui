...
const Room = ({ room }: RoomProps) => {
  ...
  const [attendersEstimation, setAttendersEstimation] =
    useState<RoomEstimation>();
  ...
  useEffect(() => {
    ...
    if (!playerName && roomInfoRaw) {
      ...
      setRoomData({
        ...
        attenders: [
          ...roomData.attenders,
          { playerName: roomInfo.playerName, isAdmin: roomInfo.isAdmin },
        ],
      });
      ...
      setRoomData({
        ...
        attenders: [
          ...roomData.attenders,
          { playerName: newPlayerName!, isAdmin: false },
        ],
      });
  ...
  const isAdmin = () => {
    const attender = roomData?.attenders.find(
  ...
  const changeRoomStatus = () => {
    return new Promise((resolve) => {
      socket.emit(
        ...
        (roomInfo: { room: RoomType; roomEstimations: any }) => {
  ...
  const emitSelectedEstimationSize = (size: string) => {
    socket.emit("sendSelectedSize", {
      roomName: roomName,
      selectedEstimationSize: size,
      playerName: playerName,
    });
  };
    ...
    <div className={roomStyles.room}>
      <div className={roomStyles.roomDirection}>
        <AttenderList
          ...
          roomEstimation={attendersEstimation}
          rooms={roomData}
        />
        <div className={roomStyles.invitedRoom}>
          ...
          <Options
            roomStatus={roomData.roomStatus}
            roomEstimation={attendersEstimation}
            cardDeck="Scrum Scale"
            selectedItem={emitSelectedEstimationSize}
          />
          <EstimateResult
            roomStatus={roomData.roomStatus}
            roomEstimations={attendersEstimation}
          />
          {isAdmin() && roomData.roomStatus !== "start" && (
            <button className="btn" onClick={toggleEstimate}>
              {roomData.roomStatus === "inprogress"
                ? `Show Result`
                : "Restart Estimation"}
            </button>
          )}
